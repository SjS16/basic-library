      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 p-4 sm:p-5">
        ...
        <div className="flex-shrink-0 mx-auto sm:mx-0">
          {book.cover_url ? (
            ...
            <div className="w-20 h-28 sm:w-24 sm:h-32 bg-gradient-to-br from-gray-200 to-gray-300 rounded shadow-sm flex items-center justify-center">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-2 gap-2">
            <div className="flex-1">
              <Link 
                to={`/books/${book.id}`}
                className="text-lg sm:text-xl font-bold text-gray-900 hover:text-blue-600 transition-colors block mb-1"
              >
                {book.title}
              </Link>
              <p className="text-gray-600 text-xs sm:text-sm mb-2">by {book.author}</p>
              
              <div className="flex items-center flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm text-gray-500 mb-3">
                {book.published_year && (
              ...
              <div className="flex items-center gap-2">
                ...
                />
                {book.rating > 0 && (
                  <span className="text-xs sm:text-sm text-gray-600">({book.rating}/5)</span>
                )}
              </div>
            </div>
